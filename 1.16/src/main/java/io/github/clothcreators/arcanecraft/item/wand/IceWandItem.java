...
public class IceWandItem extends AbstractWandItem {
	...
	public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
		if (!world.isClient() && user instanceof ServerPlayerEntity) {
			...
			ItemProjectileEntity arrow = ProjectileUtilities.shoot(ItemStack.EMPTY, world, entity, ThreadLocalRandom.current(), 0.7F, 2, 0, (hitEntity) -> hitEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 190, 2)), SoundEvents.ENTITY_EVOKER_CAST_SPELL, (arrow1) -> arrow1.world.addParticle(ParticleTypes.ITEM_SNOWBALL, arrow1.getX(), arrow1.getY(), arrow1.getZ(), 1, 1, 1), null);
